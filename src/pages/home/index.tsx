...
const Home = () => {
  ...
  <div className="absolute inset-0 pointer-events-none">
    <div className="absolute w-12 h-12 bg-white/10 rounded-full top-2 left-8 animate-float" style={{ animationDelay: "0.5s" }}></div>
    <div className="absolute w-24 h-24 bg-white/10 rounded-full top-10 left-2 animate-float" style={{ animationDelay: "0.5s" }}></div>
    <div className="absolute w-20 h-20 bg-white/10 rounded-full bottom-20 right-10 animate-float" style={{ animationDelay: "1.5s" }}></div>
    <div className="absolute w-16 h-16 bg-white/15 rounded-full top-1/2 left-1/4 animate-float" style={{ animationDelay: "3s" }}></div>
    <div className="absolute w-20 h-20 bg-white/25 rounded-full bottom-1/3 right-[36%] animate-float" style={{ animationDelay: "2s" }}></div>
  </div>
  ...
  <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10 max-w-[1600px] w-full flex flex-col lg:flex-row items-center justify-between gap-12">
    ...
    <div className="flex-1 text-center lg:text-left">
      ...
      <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black leading-tight mb-4 sm:mb-6">
        ...
        with{" "}
        <span className="text-black" style={{ fontFamily: "Times New Roman, serif" }}>
          Blink<span className="text-red-500">maid</span>
        </span>
      </h1>

      <p className="text-base sm:text-lg md:text-xl mb-6 sm:mb-8 max-w-xl sm:max-w-2xl leading-relaxed animate-fade-in" style={{ animationDelay: "0.3s", fontFamily: 'Pacifico, cursive' }}>
        Professional, trustworthy, and experienced helpers at your service anytime. Background-verified professionals ready to transform your home.
      </p>
    ...
    <div className="flex-1 flex justify-center lg:justify-end relative w-full max-w-md sm:max-w-lg lg:max-w-xl mb-8 lg:my-0">
      ...
      <img
        ...
        className="relative w-[80%] sm:w-[70%] md:w-[60%] h-auto object-cover rounded-3xl shadow-lg hover:scale-105 transition-all border-4 border-white/30 z-20"
      />

      {/* Small Service Images - Hidden on small screens */}
      <div className="hidden lg:block">
        <img
          src={maid2}
          alt="Service 2"
          className="absolute top-6 -left-6 w-44 md:w-52 rounded-3xl shadow-lg border-4 border-white/30 z-10 transition-all hover:scale-105 duration-700 ease-out"
          style={{
            transform: animate ? "translate(-5rem,-3rem) scale(1)" : "translate(0,0) scale(0.8)",
            opacity: animate ? 1 : 0,
          }}
        />
        <img
          src={maid3}
          alt="Service 3"
          className="absolute -bottom-48 -left-14 w-44 md:w-52 rounded-3xl shadow-lg border-4 border-white/30 z-10 transition-all hover:scale-105 duration-700 ease-out"
          style={{
            transform: animate ? "translate(5rem,-4rem) scale(1)" : "translate(0,0) scale(0.8)",
            opacity: animate ? 1 : 0,
          }}
        />
        <img
          src={maid4}
          alt="Service 4"
          className="absolute -top-[222px] left-28 w-32 md:w-36 rounded-3xl shadow-lg border-4 border-white/30 z-10 transition-all hover:scale-105 duration-700 ease-out"
          style={{
            transform: animate ? "translate(0,6rem) scale(1)" : "translate(0,0) scale(0.8)",
            opacity: animate ? 1 : 0,
          }}
        />
      </div>
